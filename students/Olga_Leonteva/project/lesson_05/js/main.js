...
const app = new Vue({
    ...
    data: {
        ...
        searchLine: null,
        cart: [],
        url: 'https://raw.githubusercontent.com/calabarOlga/JS_level1/master',
    ...
    async getJSON (url) {
        ...
        filterProducts () {
            ...
            return this.filteredProducts.filter(good => {return regexp.test(good.title)});
    ...
    mounted () {
        this.getJSON (`${this.url}/Catalog.json`)
        .then (data => {this.filteredProducts = data})
        .then (data => {this.productItem = data})
...
class Catalog extends List {
    ...
    filterProducts(value) {
        ...
        this.items.filter(good => regexp.test(good.product_name));
        this._render ();
...
document.querySelector('.products').addEventListener ('click', (evt) => {
    if (evt.target.classList.contains ('buy-btn')) {
       cart.addToBasket (evt.target);
    }
})

document.querySelector('.cart-block').addEventListener ('click', (evt) => {
    if (evt.target.classList.contains ('del-btn')) {
        cart.delFromBasket (evt.target);
    }
})
