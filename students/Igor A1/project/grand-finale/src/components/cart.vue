  ...
  export default {
          ...
          this.$parent.requestData(`${query.basket.add}`, 'POST', newGood)
            .then(data => {
              if(this.$parent.error)
                this.$root.$children[0].$refs.error.log(`Ошибка при добавлении товара.`)
              else {
                if(!data.result) {
                  this.$root.$children[0].$refs.error.log('Извините, этого товара больше нет в наличии.')
                } else { // success!
      ...
      removeGood(good) {
        ...
        if(good.quantity > 1) { // single remove
          this.$parent.requestData(`${query.basket.remove}`, 'PUT', {id: good.id_product, quantity: 1})
            .then(data => {
              if(this.$parent.error)
                this.$root.$children[0].$refs.error.log(`Ошибка при удалении товара.`)
              else {
                if(!data.result) {
                 this.$root.$children[0].$refs.error.log('Извините, что-то пошло не так. Попробуйте еще раз, или обратитесь в техподдержку.')
                } else { // success!
                  founded.quantity--;
                  this.cart.countGoods--;
                  this.cart.amount -= good.price;
    ...
    mounted () {
      this.$parent.loadData(`${query.basket.get}`)
        .then(data => {
          if(this.$parent.error)
            this.$root.$children[0].$refs.error.log(`Ошибка при загрузке товаров корзины.`)
          else {
            this.cart = data;
